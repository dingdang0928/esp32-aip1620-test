 ...
 */
void __attribute__((weak)) BLE_Communication_ReceDataCb(uint8_t *data,
                                                        uint16_t datalen)
{
  MY_LOGI("value = %.*s, len = %u", (int)datalen, (char *)data, datalen);
}
...
static char example_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "T6350_DINGDANG";
...
static esp_ble_adv_data_t heart_rate_adv_config = {
    ...
    .min_interval =
        ESP_BLE_GAP_CONN_ITVL_MS(7.5),  // slave connection min interval
    .max_interval =
        ESP_BLE_GAP_CONN_ITVL_MS(20),  // slave connection max interval
    .appearance = 0x00,
...
static const esp_gatts_attr_db_t heart_rate_gatt_db[HRS_IDX_NB] = {
    ...
    [HRS_IDX_HR_MEAS_VAL] = {{ESP_GATT_AUTO_RSP},
                             {ESP_UUID_LEN_16, (uint8_t *)&heart_rate_meas_uuid,
                              ESP_GATT_PERM_READ, HRPS_HT_MEAS_MAX_LEN, 0,
                              NULL}},
    ...
    [HRS_IDX_BOBY_SENSOR_LOC_VAL] =
        {{ESP_GATT_AUTO_RSP},
         {ESP_UUID_LEN_16, (uint8_t *)&body_sensor_location_uuid,
          ESP_GATT_PERM_READ_ENCRYPTED, sizeof(uint8_t),
          sizeof(body_sensor_loc_val), (uint8_t *)body_sensor_loc_val}},
    ...
    [HRS_IDX_HR_CTNL_PT_VAL] = {{ESP_GATT_AUTO_RSP},
                                {ESP_UUID_LEN_16,
                                 ...
                                 sizeof(uint8_t), sizeof(heart_ctrl_point),
...
static void show_bonded_devices(void)
{
  ...
  if (dev_num < 0)
  {
    ESP_LOGE(GATTS_TABLE_TAG,
             "Get bond device num failed (stack may be disabled), ret %d",
             dev_num);
    return;
  }
  if (dev_num == 0)
  ...
  esp_ble_bond_dev_t *dev_list = (esp_ble_bond_dev_t *)malloc(
      sizeof(esp_ble_bond_dev_t) * (size_t)dev_num);
  if (!dev_list)
  {
  ...
  switch (event)
  {
    ...
    */
    ...
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT:  /// the app will receive this evt when
      ...
      ESP_LOGI(GATTS_TABLE_TAG, "Passkey notify, passkey %06" PRIu32,
               param->ble_security.key_notif.passkey);
      break;
    case ESP_GAP_BLE_KEY_EVT:
      // shows the ble key info share with peer device to the user.
      ESP_LOGI(GATTS_TABLE_TAG, "Key exchanged, key_type %s",
                                        ...
                                        esp_ble_gatts_cb_param_t *param)
{
  ...
  switch (event)
  {
    case ESP_GATTS_REG_EVT:
      ...
      esp_ble_gatts_create_attr_tab(heart_rate_gatt_db, gatts_if, HRS_IDX_NB,
    ...
    case ESP_GATTS_WRITE_EVT:
      ...
      // 调用弱函数处理收到的数据,打印输出
      BLE_Communication_ReceDataCb(param->write.value, param->write.len);
    ...
    case ESP_GATTS_CREAT_ATTR_TAB_EVT:
    {
      if (param->add_attr_tab.status == ESP_GATT_OK)
      {
        ...
        {
          ESP_LOGE(GATTS_TABLE_TAG,
                   "Attribute table create abnormally, num_handle (%d) doesn't "
      ...
      {
        ESP_LOGE(GATTS_TABLE_TAG,
                 "Attribute table create failed, error code = %x",
...
void Dri_BLE_Init(void)
{
  ...
  memcpy(example_device_name, esp_bluedroid_get_example_name(),
